...
- **Service worker**: replace `/sw.js` with a self-destroying worker (Q3 — a 404'd script never unregisters; the legacy NetworkFirst worker would persist forever); delete `workbox-74d02f44.js` + `manifest.json` now; the self-destroyer itself dies in PR-010. *[Superseded 2026-08-10: PR-010 Q2 research reversed the deletion — sw.js is retained indefinitely (never delete a once-served SW path; convention ×3 sources). See PR-010 findings.]*
