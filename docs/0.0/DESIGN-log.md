...
**Closure (PR-010, 2026-08-10)**: all D9 bugs resolved — arrow rotate + 404-flash + Works-class keys in PR-006; resize-observer/refs in PR-004; the loadingPage setInterval leak died with the /loading route's removal (PR-010 A1, research-backed); the last missing-key instance (contact cards) fixed in PR-010's Biome sweep.
